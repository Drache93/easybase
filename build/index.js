...
function debugLog(debug, prefix, message, data) {
    if (debug) {
...
function inviteRecordToJSON(record) {
    return {
        ...record,
...
function inviteRecordFromJSON(json) {
    return {
        ...json,
...
export class EasybasePairer extends ReadyResource {
    ...
    constructor(store, invite, opts) {
        ...
        if (!store) {
            debugLog(opts.debug || false, "EasybasePairer", "ERROR: Corestore is required");
            throw new EasybasePairingError("Corestore is required");
        }
        if (!invite || typeof invite !== "string") {
            debugLog(opts.debug || false, "EasybasePairer", "ERROR: Valid invite string is required");
    ...
    async _open() {
        ...
        if (!key) {
            debugLog(this.debug, "EasybasePairer", "ERROR: Failed to get local core key");
        ...
        this.candidate = this.pairing.addCandidate({
            ...
            onadd: async (result) => {
                ...
                if (this.easybase === null) {
                    debugLog(this.debug, "EasybasePairer", "Creating new Easybase instance", {
                ...
                debugLog(this.debug, "EasybasePairer", "Waiting for Easybase to be ready");
                ...
                debugLog(this.debug, "EasybasePairer", "Joining swarm with discovery key", {
    ...
    _whenWritable() {
        ...
        if (this.easybase.base.writable) {
            debugLog(this.debug, "EasybasePairer", "Base is writable, resolving promise");
        ...
        const waitForReady = async () => {
            ...
            debugLog(this.debug, "EasybasePairer", "Base is ready, checking writable status");
            ...
            if (this.easybase.base.writable) {
                debugLog(this.debug, "EasybasePairer", "Base became writable after update, resolving promise");
        ...
        const check = () => {
            ...
            if (this.easybase?.base.writable) {
                debugLog(this.debug, "EasybasePairer", "Base became writable, resolving promise");
                this._cleanupUpdateListener();
                if (this.easybase) {
                    this.onresolve?.(this.easybase);
                }
        ...
        debugLog(this.debug, "EasybasePairer", "Added update listener for writable check");
        const interval = setInterval(async () => {
            ...
            if (this.easybase?.base.writable) {
                debugLog(this.debug, "EasybasePairer", "Base became writable during periodic check");
            ...
            try {
                await this.easybase?.base.update();
                debugLog(this.debug, "EasybasePairer", "Forced base update during periodic check", {
                    writable: this.easybase?.base.writable,
                    baseLength: this.easybase?.base.length,
                });
                if (this.easybase?.base.writable) {
                    debugLog(this.debug, "EasybasePairer", "Base became writable after forced update");
                    clearInterval(interval);
                    this._cleanupUpdateListener();
                    if (this.easybase) {
                        this.onresolve?.(this.easybase);
...
export class Easybase extends ReadyResource {
        ...
        if (this.actions) {
            ...
            for (const [actionName, actionFn] of Object.entries(this.actions)) {
                this[actionName] = async (value) => {
                    debugLog(this.debug, "Easybase", `Dynamic action called: ${actionName}`, { value });
    ...
    async _delInvite(view, record) {
        ...
        if (this._isHyperdrive(view)) {
            ...
            debugLog(this.debug, "Easybase", "ERROR: Cannot delete invite from default view");
            throw new EasybaseError("Cannot delete invite from default view", "INVALID_OPERATION");
        }
    }
            ...
            switch (type) {
                ...
                default:
                    ...
                    if (this.actions && type in this.actions) {
                        ...
                        throw new EasybaseError(`Unknown action: ${type}`, "UNKNOWN_ACTION");
                    }
                    break;
            }
        }
    ...
    async _open() {
        ...
        this.swarm.on("connection", (connection, peerInfo) => {
            debugLog(this.debug, "Easybase", "Swarm connection established during replication", {
                peerInfo,
            });
    ...
    async _close() {
        ...
        if (this.viewType === "hyperdrive" && this.base.view) {
            ...
            try {
                ...
                debugLog(this.debug, "Easybase", "Warning: Error closing Hyperdrive view", { error });
                // Log error but don't throw - this is expected behavior on first create
                console.warn("Warning: Error closing Hyperdrive view (this may be expected):", error);
            }
        }
    ...
    async getInvite() {
        ...
        if (this._isHyperdrive(this.base.view)) {
            ...
            try {
                ...
                debugLog(this.debug, "Easybase", "Error getting invite from Hyperdrive", { error });
    ...
    async deleteInvite() {
        ...
        let existing = null;
        if (this._isHyperdrive(this.base.view)) {
            debugLog(this.debug, "Easybase", "Getting invite from Hyperdrive for deletion");
            try {
                const data = await this.base.view.get("invite.json");
                if (data) {
                    existing = inviteRecordFromJSON(JSON.parse(data.toString("utf-8")));
                    debugLog(this.debug, "Easybase", "Found invite to delete in Hyperdrive");
                }
            }
            catch (error) {
                debugLog(this.debug, "Easybase", "Error getting invite from Hyperdrive for deletion", { error });
                // Ignore errors when invite doesn't exist
            }
        }
        else {
            debugLog(this.debug, "Easybase", "Getting invite from Hyperbee for deletion");
            try {
                existing = await this.base.view.findOne("@easybase/invite", {});
                if (existing) {
                    debugLog(this.debug, "Easybase", "Found invite to delete in Hyperbee");
                }
            }
            catch (error) {
                debugLog(this.debug, "Easybase", "Error getting invite from Hyperbee for deletion", { error });
                // Ignore errors when invite doesn't exist
            }
        }
        if (existing) {
    ...
    async _replicate() {
        ...
        if (this.swarm === null) {
            debugLog(this.debug, "Easybase", "Creating new Hyperswarm for replication");
            this.swarm = new Hyperswarm();
            this.swarm.on("connection", (connection, peerInfo) => {
                debugLog(this.debug, "Easybase", "Swarm connection established during replication", {
                    peerInfo: peerInfo ? { id: peerInfo.id?.toString("hex") } : null,
